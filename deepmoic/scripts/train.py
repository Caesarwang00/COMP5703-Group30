...
def main(config: Cfg) -> None:
    ...
    metric = str(snf_cfg.get("METRIC", "cosine"))
    K = int(snf_cfg.get("K", max(5, min(20, int(0.3 * n_samples)))))
    ...
    train_ratio = float(exp_cfg.get("TRAIN_RATIO", 0.7))
    repeats     = int(exp_cfg.get("REPEATS", 5))
    ...
    # === 新增：整体汇总器（覆盖 train/val/test 三个拆分）===
    acc_tr_list, acc_val_list, acc_te_list = [], [], []
    per_class_correct = {
        "train": np.zeros(n_classes, dtype=np.int64),
        "val":   np.zeros(n_classes, dtype=np.int64),
        "test":  np.zeros(n_classes, dtype=np.int64),
    }
    per_class_total = {
        "train": np.zeros(n_classes, dtype=np.int64),
        "val":   np.zeros(n_classes, dtype=np.int64),
    ...
    for rep_idx, (tr_idx, te_idx) in enumerate(sss_outer.split(np.zeros(n_samples), y), start=1):
        ...
        # 预测取 numpy
        p_all = pred_all.detach().cpu().numpy() if isinstance(pred_all, torch.Tensor) else np.asarray(pred_all)

        # 拆分预测/标签
        y_tr, y_val, y_te = y[tr_mask], y[val_mask], y[te_mask]
        p_tr, p_val, p_te = p_all[tr_mask], p_all[val_mask], p_all[te_mask]

        # 整体准确率
        acc_tr = float((p_tr == y_tr).mean()) if y_tr.size else float("nan")
        acc_val = float((p_val == y_val).mean()) if y_val.size else float("nan")
        acc_te = float((p_te == y_te).mean())
        acc_tr_list.append(acc_tr); acc_val_list.append(acc_val); acc_te_list.append(acc_te)

        # 原有：以 test 为基准的评估（含 macro-F1）
        m = evaluate_cls(y_true=y_te, y_pred=p_te)
        print(f"[REP {rep_idx}] train_acc={acc_tr:.4f} val_acc={acc_val:.4f} test_acc={acc_te:.4f} macro_f1={m.get('f1',0):.4f}")
        all_metrics.append(m)

        # === 新增：逐类正确/总数累计（train/val/test）===
        for c in range(n_classes):
            sel = (y_tr == c)
            per_class_total["train"][c]   += int(sel.sum())
            per_class_correct["train"][c] += int((p_tr[sel] == c).sum())

            sel = (y_val == c)
            per_class_total["val"][c]     += int(sel.sum())
            per_class_correct["val"][c]   += int((p_val[sel] == c).sum())

            sel = (y_te == c)
            per_class_total["test"][c]    += int(sel.sum())
            per_class_correct["test"][c]  += int((p_te[sel] == c).sum())
    ...
    if all_metrics:
        ...
        print("========== SUMMARY ==========")
        print(f"acc(test) : {np.mean(accs):.4f} ± {np.std(accs):.4f}")
        print(f"maF1(test): {np.mean(macro_f1s):.4f} ± {np.std(macro_f1s):.4f}")

        # 额外给出三拆分的整体 acc（跨 repeats）
        def _nanmeanstd(a):
            a = np.asarray(a, dtype=float); a = a[~np.isnan(a)]
            return (np.nan, np.nan) if a.size == 0 else (a.mean(), a.std())
        mean_tr, std_tr = _nanmeanstd(acc_tr_list)
        mean_val, std_val = _nanmeanstd(acc_val_list)
        mean_te, std_te = _nanmeanstd(acc_te_list)
        print(f"acc(train): {mean_tr:.4f} ± {std_tr:.4f}")
        print(f"acc(val)  : {mean_val:.4f} ± {std_val:.4f}")
        print(f"acc(test) : {mean_te:.4f} ± {std_te:.4f}")

        # === 新增：逐类正确率（Train/Val/Test，合并所有 repeats）===
        def _print_per_class(split_name: str):
            print(f"========== PER-CLASS ACC ({split_name.upper()}, pooled over repeats) ==========")
            for i, cname in enumerate(names):
                tot = per_class_total[split_name][i]
                cor = per_class_correct[split_name][i]
                if tot > 0:
                    pc = cor / tot
                    print(f"- {cname}: {pc:.4f}  ({cor}/{tot})")
                else:
                    print(f"- {cname}: NaN  (该类在所有 {split_name} 划分中均无样本)")
        _print_per_class("train")
        _print_per_class("val")
        _print_per_class("test")
