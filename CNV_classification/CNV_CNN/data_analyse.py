...
def main():
    ...
    if sid_col is None:
        raise RuntimeError("clinical lack sampleID / bcr_sample_barcode")
    ...
    for key in ["cnv_thres", "mutation", "methyl450", "mirna"]:
        ...
        print(f"lack value rate: {rep['nan_pct']}% | zero raw: {rep['all_zero_rows']} | zero col: {rep['all_zero_cols']} | 0 value rate: {rep['zero_pct_overall']}%")
        if rep["all_zero_cols"] > 0:
            zero_cols = df_num.columns[(df_num.fillna(0).abs().sum(axis=0) == 0)].tolist()[:5]
            print("zero sample:", zero_cols)
        if abn:
            print("rule check:", abn)
        print("Worst sample (lack rate Top5):", rep["worst_cols_by_na"])

        # 收集样本短ID
        sample_sets[key] = extract_shorts_from_cols(df_num)
        print("Sample num:", len(sample_sets[key]))
