...
- 路径解析更健壮：优先环境变量，其次父级 data 目录，最后本地 data 目录
- 关键超参统一整理并标注（★）
"""
...
# 优先使用环境变量 GBM_DATA_DIR；否则尝试父级 data（…/COMP5703Group30/data），再退到本地 data
_DATA_CANDIDATES = [
    (os.environ.get("GBM_DATA_DIR") or "").strip() or None,
    os.path.abspath(os.path.join(PROJECT_ROOT, "..", "data")),   # 父级 data（推荐你的项目结构）
    os.path.abspath(os.path.join(PROJECT_ROOT, "data")),         # 本地 data（备用）
]
