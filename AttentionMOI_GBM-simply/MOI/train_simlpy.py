...
def setup_logging():
    ...
    logging.basicConfig(
        level=logging.INFO,  # 修正这里：logging.issued -> logging.INFO
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        attended = self.layer_norm(attended.sum(dim=1))  # 添加LayerNorm
        return attended


class AttentionMOI(nn.Module):
    def __init__(self, dna_features, rna_features, num_classes, hidden_dim=128, dropout_rate=0.5, activation='gelu'):
        super().__init__()
        act = nn.GELU() if activation == 'gelu' else nn.ReLU() if activation == 'relu' else nn.Tanh()

        # DNA pathway - 添加BatchNorm
        ...
        self.dna_mlp = nn.Sequential(
            ...
            act,
            nn.Dropout(dropout_rate),  # 使用配置的dropout率
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.BatchNorm1d(hidden_dim // 2),
            act,
            nn.Dropout(dropout_rate)  # 添加额外的dropout
        )

        # RNA pathway - 添加BatchNorm
        ...
        # Fusion and classification - 添加更多正则化
        self.cross_attention = AttentionLayer(hidden_dim // 2, hidden_dim // 2)
        self.fusion_norm = nn.LayerNorm(hidden_dim // 2)  # 添加LayerNorm
    ...
    def forward(self, dna_data, rna_data):
        ...
        fused = self.fusion_norm(fused)  # 应用LayerNorm
...
class RegularizedAttentionMOITrainer:
    ...
    def compute_l2_penalty(self, model):
        """计算L2正则化惩罚项"""
        l2_penalty = 0
    ...
    def train_epoch(self, dataloader, optimizer, criterion, weight_decay):
        ...
        for dna, rna, labels in dataloader:
            ...
            l2_penalty = weight_decay * self.compute_l2_penalty(self.model)
    ...
    def train(self, train_loader, val_loader, cfg):
        ...
        # Optimizer selection
        optimizers = {
            'adam': torch.optim.Adam(self.model.parameters(), lr=cfg['lr'], weight_decay=0),
            'adamw': torch.optim.AdamW(self.model.parameters(), lr=cfg['lr'], weight_decay=0),
            'sgd': torch.optim.SGD(self.model.parameters(), lr=cfg['lr'], weight_decay=0, momentum=cfg['momentum'])
        }
        ...
        for epoch in range(cfg['epochs']):
            train_loss = self.train_epoch(train_loader, optimizer, criterion, cfg['weight_decay'])
            ...
            # 逐类准确率打印（C0..C{K-1}；若某类当轮没样本会显示 NA）
            pc_val_str = " | ".join(
...
def main(cfg):
    ...
    logger.info("Regularized Attention-MOI Training Started")
