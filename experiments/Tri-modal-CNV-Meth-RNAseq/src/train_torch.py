...
def train_one_fold(Xtr, ytr, Xva, yva, labels, fold_dir, device):
    ...
    for epoch in iterator:
        ...
        if f1m > best_f1 + 1e-6:
            ...
            best_state = {"models": model.state_dict(), "meta": {"in_dim": in_dim, "n_class": n_class, "hidden_dim": HIDDEN_DIM, "dropout": DROPOUT}}
        else:
            no_improve += 1
            if no_improve >= PATIENCE: break

    if best_state is not None: model.load_state_dict(best_state["models"])
    ...
    # 保存模型
    torch.save(best_state if best_state is not None else {"models": model.state_dict()}, os.path.join(fold_dir, "models.pt"))
    return y_pred
